...
public class InfobipSmsService : ISpeedSmsService
{
    ...
    {
        var options = _configuration.GetSection("Infobip").Get<InfobipOptions>() ?? new InfobipOptions();
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, $"{options.BaseUrl.TrimEnd('/')}/sms/2/text/advanced");
        ...
        {
            throw new InvalidOperationException($"Infobip gửi OTP thất bại. Status: {(int)response.StatusCode}. Chi tiết: {body}");
        }

        var result = JsonSerializer.Deserialize<InfobipSendResponse>(body, new JsonSerializerOptions
        {
        ...
        {
            throw new InvalidOperationException("Infobip không trả về thông tin tin nhắn nào.");
        }

        var failedMessage = result.Messages.FirstOrDefault(m => IsFailedStatus(m.Status));
        if (failedMessage != null)
        {
            throw new InvalidOperationException($"Infobip từ chối gửi OTP. Chi tiết: {JsonSerializer.Serialize(failedMessage.Status)}");
        }
    }
    ...
    private static bool IsFailedStatus(InfobipStatus? status)
    {
        ...
        var groupName = (status.GroupName ?? string.Empty).Trim();
