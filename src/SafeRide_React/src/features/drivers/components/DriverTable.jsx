...
import { faSlidersH, faCheck, faEye, faLock, faLockOpen, faStar as faStarSolid } from '@fortawesome/free-solid-svg-icons';
...
const PAGE_SIZE = 10;
function DriverTable({ drivers, counts, activeTab, onTabChange, onSelectDriver, onToggleBlock, actionDriverId, }) {
    ...
    return (<div className="driver-table-container" id="driver-table-container">
      {/* Tabs + Filter */}
      <div className="driver-table-toolbar">
        <div className="driver-tabs" id="driver-tabs">
          {TABS.map((tab) => (<button key={tab.id} type="button" className={`driver-tab${activeTab === tab.id ? ' driver-tab--active' : ''}`} onClick={() => handleTabChange(tab.id)}>
              {tab.label}
            ...
            return (<tr key={driver.id}>
                  ...
                  <td className="col-actions">
                    <div className="actions-cell">
                      {driver.status === 'pending_kyc' ? (<button type="button" className="action-link action-link--teal" onClick={() => onSelectDriver?.(driver)}>
                          <FontAwesomeIcon icon={faCheck}/>
                          Duyệt KYC
                        </button>) : (<button type="button" className="action-link action-link--teal" onClick={() => onSelectDriver?.(driver)}>
                          <FontAwesomeIcon icon={faEye}/>
                          Chi tiết
                        </button>)}
                      <button type="button" className={`action-link ${driver.isActive ? 'action-link--red' : 'action-link--teal'}`} onClick={() => onToggleBlock?.(driver)} disabled={isActionBusy}>
                        <FontAwesomeIcon icon={driver.isActive ? faLock : faLockOpen}/>
                        {driver.isActive ? 'Khóa' : 'Mở khóa'}
                      </button>
                    </div>
                  </td>
                </tr>);
        })}
            {pageDrivers.length === 0 && (<tr>
